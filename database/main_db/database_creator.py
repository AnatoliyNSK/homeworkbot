...
def fill_db_from_files(disciplines_path: str, excel_path: str) -> None:
    configurator = FirstRunConfigurator(disciplines_path, excel_path)
    ...
    for it in configurator.students_config:
        for group_name, student_raw_list in configurator.students_config[it].items():
            temp_students[it] = []
    ...
    for dis, teacher_group in configurator.teachers_config.items():
        for group_name, teacher_raw_list in teacher_group.items():
            ...
            for teacher_raw in teacher_raw_list:
                teacher = Teacher(
                    full_name=teacher_raw.full_name,
                    telegram_id=teacher_raw.telegram_id
                )
                if teacher_raw.is_admin:
                    session.add(Admin(telegram_id=teacher.telegram_id))
                session.add(teacher)
                session.flush()
                session.add(
                    TeacherGroup(
