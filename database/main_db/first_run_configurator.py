...
class FirstRunConfigurator:
    ...
    def __create_directory(self):
        """
        Создает директории дисциплин, куда будут помещаться тесты, которые будет загружать админ,
        а ткже ответы студентов
        :return: None
        """
        path = Path.cwd()
    ...
    def disciplines(self) -> list[DisciplineWorksConfig]:
        """
        :return: список из конфигураций дисциплин
        (для каждой дисциплины, будет своя конфигурация)
        """
